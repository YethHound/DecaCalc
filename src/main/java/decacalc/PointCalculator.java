...
public class PointCalculator{
	...
	private static Event longJump = new Event("Long_jump",false,0.14354,220,1.4);
	private static Event shotPut = new Event("Shot_put",false,51.39,1.5,1.05);
	private static Event highJump = new Event("High_jump",false,0.8465,75,1.42);
	private static Event fourHundredMeterDash = new Event("400m",true,1.53775,82,1.81);
	private static Event hurdles = new Event("110m_hurdles",true,5.74352,28.5,1.52);
	private static Event discus = new Event("Discus_throw",false,12.91,4,1.1);
	private static Event poleVault = new Event("Pole_vault",false,0.2797,100,1.35);
	private static Event javelin = new Event("Javelin_throw",false,10.14,7,1.08);
	...
	public static double points(String eventName, Double result){
		...
		if(events.get(eventName)==null){
			throw new IllegalArgumentException("Valid parameters are: '100m', 'Long_jump','Shot_put','High_jump','400m','110m_hurdles','Discus_throw','Pole_vault','Javelin_throw' and '1500m'");
		}
